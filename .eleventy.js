...
module.exports = function (eleventyConfig) {
  ...
  eleventyConfig.addNunjucksShortcode("p5gallery", function (folder) {
    ...
    if (!fs.existsSync(absFolder)) {
      // Folder does not exist, skip rendering
      return "";
    }
    const files = fs.readdirSync(absFolder).filter((f) => f.endsWith(".js"));
    if (files.length === 0) {
      // No scripts, skip rendering
      return "";
    }
    let html = "";
  ...
  ${script}
      ...
      html += `
                 <div class=\"p5-block\" style=\"margin-bottom:2em;\">
                   ...
                   <pre id=\"p5-code-${idx}\" style=\"display:none;background:#222;color:#fff;padding:1em;overflow:auto;margin:0;\">${script
        .replace(/</g, "&lt;")
        .replace(/>/g, "&gt;")}</pre>
                 </div>
               `;
